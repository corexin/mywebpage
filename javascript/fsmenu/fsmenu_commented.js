...
{
 // Backup original margin, then 'swipe' margin + clipping.
 if (show && (counter == 0))
 {
  ref._fsm_styMT = ref.style.marginTop;
  ref._fsm_curMT = parseInt(ref.currentStyle ? ref.currentStyle.marginTop :
   (document.defaultView && document.defaultView.getComputedStyle ?
    document.defaultView.getComputedStyle(ref, null).getPropertyValue('marginTop') : 0));
 }
 ...
 {
  if (ref._fsm_styMT) ref.style.marginTop = ref._fsm_styMT;
  else if (ref.style.removeProperty) ref.style.removeProperty('margin-top', true);
  else if (ref.style.cssText)
   ref.style.cssText = ref.style.cssText.replace(/MARGIN-TOP:[^;]*;\s*/i, '');
 }
 else ref.style.marginTop = ((0 - clipY) + (ref._fsm_curMT || 0)) + 'px';
};
